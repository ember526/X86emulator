...
void difftest_step(uint32_t eip) {
  ...
  if(ref_r.eip != cpu.eip) {
    ...
    printf("problem with $eip at 0x%x in nemu, when $eip is 0x%x in qemu.\n", cpu.eip, ref_r.eip);
  }
  for (int i = R_EAX; i <= R_EDI; i ++) {
    if(ref_r.gpr[i]._32 != cpu.gpr[i]._32) {
      printf("Problem with $%s (0x%x) at 0x%x in nemu, when that reg is 0x%x in qemu.\n", regsl[i], cpu.gpr[i]._32, cpu.eip, ref_r.gpr[i]._32);
      nemu_state = NEMU_ABORT;
    }
  }
}
